        ...
        loop {
            ...
            if nodes % 16384 == 0 {
                ...
                if let Some(time) = limits.opt_time {
                    ...
                    let falling_eval = (1.0 + eval_diff * self.params.tm_falling_eval1()).clamp(self.params.tm_falling_eval2(), self.params.tm_falling_eval3());

                    // Use more time if our best move is changing frequently
                    let best_move_instability =
                        (1.0 + (best_move_changes as f32 * self.params.tm_bmi1()).ln_1p()).clamp(self.params.tm_bmi2(), self.params.tm_bmi3());
                    ...
                    let best_move_visits =
                        (self.params.tm_bmv1() - ((nodes_effort + self.params.tm_bmv2()) * self.params.tm_bmv3()).ln_1p() * self.params.tm_bmv4()).clamp(self.params.tm_bmv5(), self.params.tm_bmv6());
