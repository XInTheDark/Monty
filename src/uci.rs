...
pub fn run(policy: &PolicyNetwork, value: &ValueNetwork) {
    ...
    loop {
        ...
        match cmd {
            ...
            "go" => {
                // increment game ply on each go command
                root_game_ply += 2;
            ...
            "policy" => {
                ...
                // Sort moves in descending probability order.
...
pub fn bench(depth: usize, policy: &PolicyNetwork, value: &ValueNetwork, params: &MctsParams) {
    ...
    let bench_fens = [
        ...
        // ... (other FEN strings omitted for brevity)
    ];

    let limits = crate::mcts::Limits {
        max_time: None,
        opt_time: None,
        max_depth: depth,
        max_nodes: 1_000_000,
    ...
    for fen in bench_fens.iter() {
        ...
        let searcher = Searcher::new(&tree, params, policy, value, &abort, 1);
...
fn go(
    ...
    // If time controls are given, compute optimum time.
    ...
    // Pass threads to the Searcher so that it can limit tree updates and start evaluator threads.
    let searcher = Searcher::new(tree, params, policy, value, &abort, threads);
